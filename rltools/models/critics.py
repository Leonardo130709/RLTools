...
from .common import QuantileEmbedding, LayerNormTanhMLP
nn = torch.nn
...
class Critic(nn.Module):
    def __init__(self, in_features: int, layers: tuple, heads=2):
        super().__init__()
        self.qs = nn.ModuleList([LayerNormTanhMLP(in_features, *layers, 1) for _ in range(heads)])
...
class DistributionalCritic(nn.Module):
    def __init__(self, in_features: int, layers: tuple, quantile_emb_dim: int = 64):
        ...
        self.mlp = LayerNormTanhMLP(in_features, *layers, 1)
