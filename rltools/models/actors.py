...
class CategoricalActor(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int, act_number: int,
                 layers: tuple = (32, 32), ordinal: bool = False):
        super().__init__()
        self.act_dim = act_dim
        self.act_num = act_number
        self.ordinal = ordinal
        self.mlp = LayerNormTanhMLP(obs_dim, *layers, act_dim*act_number)
...
class ContinuousActor(nn.Module):
    def __init__(self, obs_dim: int, act_dim: int, layers: tuple,
                 mean_scale: float = 1, init_std: float = 1.):
        super().__init__()
        self.mean_scale = mean_scale
        self.mlp = LayerNormTanhMLP(obs_dim, *layers, 2*act_dim)
